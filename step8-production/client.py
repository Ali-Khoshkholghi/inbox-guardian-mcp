...
Carries Step 7's client forward unmodified in every actual capability
(see demonstrate_progress/demonstrate_cancellation/sampling_handler/
elicitation_handler below) -- Step 8 is a hardening pass on the server
this client talks to, not a new client-side primitive. What's new here
is purely diagnostic: printing the two things a stranger would want to
see to trust the connection without reading server.py --
`serverInfo.version` (the exact git SHA the server is running, see
`main()` below) and each tool's `annotations` from `tools/list` (proof
the safety hints are actually present on the wire, not just documented).

`sampling_handler` and `elicitation_handler` look superficially similar
-- both are server-initiated, both run mid-`tools/call` -- but they are
not the same primitive, and this file keeps them as two separate
...
# Configurable verbosity, same discipline as server.py: default INFO, opt
# into DEBUG (raw content of every server-initiated request this client
# fulfills) via LOG_LEVEL, not a hardcoded level.
...
logger = logging.getLogger("inbox-guardian.client")
...
def _make_oauth_auth() -> OAuthClientProvider:
    ...
    return OAuthClientProvider(
        ...
        client_metadata=OAuthClientMetadata(
            client_name="inbox-guardian-client",
...
async def demonstrate_progress(session: ClientSession, file_id: str) -> None:
    ...
    # structuredContent (Step 8) is the schema-validated dict itself, not
    # text this client has to re-parse -- content[0].text still carries
    # the same data (the SDK auto-generates it via json.dumps) for a
    # client that only understands the older text-content convention.
    print(f"  final result (structuredContent, schema-validated): {result.structuredContent}")
                ...
                types.ClientNotification(
                    types.CancelledNotification(
                        params=types.CancelledNotificationParams(
                            ...
                            reason="demonstrating cancellation (Step 8 acceptance test)",
                        )
                    )
                )
            )
...
async def _search_drive_files(session: ClientSession, query: str) -> dict | None:
    """Call search_drive_files and return its structuredContent, or None if
    ...
    The result is always one shape (Step 8's SearchDriveFilesResult,
    published as the tool's outputSchema): `{"status": "matched",
    "matches": [...], "message": None}` or `{"status": "ambiguous",
    "matches": [...unresolved candidates...], "message": "..."}` -- a
    single schema-validated shape, not the bare-list-vs-dict guessing
    game Step 7 returned. `result.structuredContent` is read directly
    rather than re-parsing `result.content[0].text`, now that the server
    publishes it natively.
    """
...
async def demonstrate_sampling_ranking(session: ClientSession) -> list[dict] | None:
    ...
    if outcome["status"] != "matched":
        raise RuntimeError(f"expected an unambiguous match for {query!r}, got: {outcome}")
...
async def demonstrate_elicitation_ambiguity(session: ClientSession) -> None:
    ...
    for i, match in enumerate(outcome["matches"], start=1):
        print(f"  {i}. {match['name']} ({match['mimeType']})")
    print()
...
async def main() -> None:
    ...
    async with streamablehttp_client(SERVER_URL, auth=auth) as (read, write, _get_session_id):
        async with ClientSession(
            ...
            # serverInfo.version is this step's git-SHA versioning (see
            # server.py's _resolve_git_sha) -- printed here because "which
            # exact commit is this server running" is the whole point of
            # embedding it, and a client is where that question actually
            # gets asked.
            server_info = init_result.serverInfo
            print(f"    serverInfo: name={server_info.name!r} version={server_info.version!r}\n")

            tools = await session.list_tools()
            print("=== tools/list: annotations alone, no source file read ===")
            for tool in tools.tools:
                a = tool.annotations
                if a is None:
                    verdict = "UNKNOWN -- no annotations published, can't tell without reading source"
                else:
                    verdict = (
                        "safe to call automatically (read-only)"
                        if a.readOnlyHint
                        else "REQUIRES CONFIRMATION before calling (destructiveHint="
                        f"{a.destructiveHint}, not read-only)"
                    )
                print(f"  - {tool.name}")
                print(
                    f"      readOnlyHint={a.readOnlyHint} destructiveHint={a.destructiveHint} "
                    f"idempotentHint={a.idempotentHint} openWorldHint={a.openWorldHint}"
                    if a is not None
                    else "      (no annotations)"
                )
                print(f"      => {verdict}")
